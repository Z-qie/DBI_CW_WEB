...
<?php
    ...
    $query = "SELECT * FROM Orders WHERE post_status = 'abnormal' OR post_status = 'abnormal to be reviewed' ORDER BY order_ID DESC";
    ...
    while ($row = $stmt -> fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($row['post_status'] == 'abnormal to be reviewed') {
            // set list
            echo 
            "<div class='order_list' style='top: " . (15 + $count * 30) . "%;'>" .
                ...
                // alert manager abnormal orders to be reviewed
                "<div class='status' style='color: red;  text-shadow: 0 0 5px red; '>" . $row['post_status'] ."</div>" .
     ...
     echo 
     "</div> <div class='title_fixed'>abnormal order (sold): &nbsp; " . $count . " in total</div>";
     $conn = null;
?>
